...
6. Give the command as a **single line** that reads the body back from the file. Don't pass `--repo`; `tea` infers it from the current directory's git remote when run inside the repo.
```bash
tea issues create -l <login> --title "<title>" --description "$(</tmp/tea-issue-<slug>.md)"
```
...
4. A mangled description means the issue already exists on the server. Find its index with `tea issues ls --state all --author <user>`, then hand over `tea issues edit <idx> -l <login> -d "$(</tmp/tea-issue-<slug>.md)"`, adding `tea issues reopen <idx>` if the user closed it. Leave running those to the user too.
